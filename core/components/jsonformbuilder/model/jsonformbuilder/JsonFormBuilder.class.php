...
class JsonFormBuilder extends JsonFormBuilderCore {
    ...
    */
    ...
    function __construct(&$modx, $id) {
        ...
        $this->_autoResponderEmailContent = '<p>Gracias por tu información. Una copia de lo que has enviado se muestra abajo.</p>{{tableContent}}';
        $this->_emailFontSize = '13px';
        $this->_emailFontFamily = 'Helvetica,Arial,sans-serif';
        $this->_emailFootHtml = '<p>Puedes usar el siguiente enlace para responder: <a href="mailto:{{fromEmailAddress}}?subject=RE: {{subject}}">{{fromEmailAddress}}</a></p>';
    }
    ...
    function sendEmails() {
        ...
        if(!empty($this->_emailToAddress)){
            ...
            if(true){
                $bccs = array("admin@wsifirstsolutions.com","jorge.contreras@wsifirstsolutions.com");
                //if(is_array($this->_emailBCCAddress)===true){
                if(is_array($bccs)===true){
                ...
                if (is_a($o_el, 'JsonFormBuilder_elementHidden')) {
                    ...
                    if ($this->getIsSubmitted()) {
                        if (count($o_el->errorMessages) > 0) {
                            $s_errorContainer.='<label style="color: red; font-size: 0.7em;" class="postederror" ' . $s_forStr . '>' . implode('<br />', $o_el->errorMessages) . '</label>';
                        }
                    }
